...
def main():
    ...
    ilist = ['Estado.csv']
    tlist = ['ESTADO DE LA SEQUIA METEOROLOGICA DE CORTO PERIODO']
...
#    plt.title("Mapa de "+tlist+" \ncorrespondiente al mes de "+mes, fontsize=14, fontweight='bold')
...
#    colors = ('#ffffff','#ffff00','#a87000')
    colors = ('#ffffff','#ffff00','#a87000')
    clevs= [-1.,0.,1.,2.]
    cmap1 = LinearSegmentedColormap.from_list("my_colormap", colors, N=3, gamma=1.0)

    cs = m.contourf(xi,yi,newout,clevs,cmap=cmap1, extend='both')

    cs.cmap.set_under('#ffffff')
    cs.cmap.set_over('#a87000')
...
#    cb = mpl.colorbar.ColorbarBase(ax2, cmap=cmap1, spacing='uniform', ticks=clevs, boundaries=clevs, format='%2.1f')
    #cb.set_label('[Valores del indice]', rotation=0, labelpad=-45)

    ax2 = fig.add_axes([0.245, 0.33, 0.1, 0.06])
    ...
    plt.text(0.22, 0.299,"AS       CS       SQ", ha='center', va='center', fontsize=11, transform=ax.transAxes)

    plt.text(0.22, 0.20,"CS: CONDICIONES FAVORABLES PARA LA SEQUIA\nAS: AUSENCIA DE SEQUIA    SQ: SEQUIA", ha='center', va='center', fontsize=10, transform=ax.transAxes)
